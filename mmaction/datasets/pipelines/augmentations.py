...
class KeyframeRandomResize(object):
    def __init__(self, sizes, max_size=None):
        ...
        try:
            ...
            raise RuntimeError('Install torchvision to use KeyframeAug')
        ...
        def resize(image, target, size, max_size=None):
            ...
            # size = (size, size)
            rescaled_image = tv_trans_F.resize(image, size)
...
class KeyframeRandomSizeCrop(object):
    def __init__(self, min_size: int, max_size: int):
        ...
        def crop(image, target, region):
            cropped_image = tv_trans_F.crop(image, *region)
    ...
    def __call__(self, img, target: dict):
        ...
        region = self.T.RandomCrop.get_params(img, [h, w])
        return self.crop(img, target, region)

class KeyframeRandomHorizontalFlip(object):
    def __init__(self, p=0.5):
        ...
        def hflip(image, target):
            flipped_image = tv_trans_F.hflip(image)
    ...
    def __call__(self, img, target):
        if random.random() < self.p:
            return self.hflip(img, target)
        return img, target

class KeyframeCompose(object):
    def __init__(self, transforms):
        self.transforms = transforms
...
class KeyframeNormalize(object):
    def __init__(self, mean, std):
        ...
        self.normalize = tv_trans_F.normalize
    ...
    def __call__(self, image, target=None):
        ...
        if "boxes" in target:
            boxes = target["boxes"]
            target["boxes_xyxy_wo_norm"] = boxes
            boxes = self.box_xyxy_to_cxcywh(boxes)
            target["boxes_cxcywh_wo_norm"] = boxes
            boxes = boxes / torch.tensor([w, h, w, h], dtype=torch.float32)
            target["boxes"] = boxes
        return image, target
    
class KeyframeToTensor(object):
    def __init__(self):
        try:
            import torchvision.transforms.functional as tv_trans_F
        except ImportError:
            raise RuntimeError('Install torchvision to use KeyframeAug')
        self.to_tensor = tv_trans_F.to_tensor
        
    def __call__(self, img, target):
        return self.to_tensor(img), target

class KeyframeRandomSelect(object):
    """
...
@PIPELINES.register_module()
class KeyframeAug:
    """KeyframeAug. Augmentation of keyframe in train pipeline.

    TODO Required keys are "audios", "sample_rate", "num_clips", added or modified
    keys are "audios".
    ...
    def __init__(self, mode='train'):
        ...
        try:
            ...
            raise RuntimeError('Install PIL to use KeyframeAug')
        self.Image = Image
        
    def __call__(self, results):
        """Perform KeyframeAugTrain transformation.
        ...
        normalize = KeyframeCompose([
            KeyframeToTensor(),
            KeyframeNormalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        ])
        ...
        if self.mode == 'train':
            trans = KeyframeCompose([
                KeyframeRandomHorizontalFlip(),
                KeyframeRandomSelect(
                    KeyframeRandomResize(scales, max_size=scales[-1] * 1333 // 800),
                    KeyframeCompose([
                        KeyframeRandomResize([400, 500, 600]),
                        KeyframeRandomSizeCrop(384, 600),
                        KeyframeRandomResize(scales, max_size=scales[-1] * 1333 // 800),
                    ])
                ),
                normalize,
            ...
            trans = KeyframeCompose([
                KeyframeRandomResize([800], max_size=800 * 1333 // 800),
                normalize,
            ])
        
        keyframe = results['keyframe']
        assert isinstance(keyframe, np.ndarray)
        keyframe = self.Image.fromarray(keyframe)
        num_objs = results['gt_labels'].shape[0]
        
        target = dict(
            boxes = torch.as_tensor(results['gt_bboxes']),
            labels = torch.zeros([num_objs], dtype=torch.int64)
        )
        keyframe, target = trans(keyframe, target)
        
        results['keyframe'] = keyframe
        results['keyframe_shape'] = target['size']
        results['keyframe_gt_person_bboxes'] = target['boxes_cxcywh_wo_norm']
        results['keyframe_gt_person_labels'] = target['labels']
        results['keyframe_gt_person_bboxes_xyxy_wo_norm'] = target["boxes_xyxy_wo_norm"]
        
        if 'keep' in target:
            results['keyframe_gt_keepinds_after_crop'] = target['keep']
        
        if 'keyframe_scale_factor' not in results:
            results['keyframe_scale_factor'] = np.array([1, 1], dtype=np.float32)
        img_w, img_h = results['original_shape']
        new_w, new_h = results['keyframe_shape'].numpy()
        results['keyframe_scale_factor'] = np.array([new_w / img_w, new_h / img_h], dtype=np.float32) 
        results['keyframe_keep_ratio'] = True 
        return results
